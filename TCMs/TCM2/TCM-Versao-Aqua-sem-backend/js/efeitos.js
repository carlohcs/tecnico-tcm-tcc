...
$(document).ready(function(){
	...
	$('#bg_cbanner ul').cycle({
		...
		before: function(atual, proximo, opcoes, avancando){
			...
			ativo = $("li.activeSlide a").text();
			
			//alert(eval(ativo));

			if(ativo == '') ativo = "#btn_cbanner1";
			else ativo = "#btn_cbanner"+(parseInt(ativo)+ 1);
			
			for(i=1; i < 5; i++){
				id = "#btn_cbanner"+i;

				if(id == ativo)	$(ativo).css('display', 'block');
	 ...
	 function pagerFactory(idx, slide){
		...
		//$('#btn_cbanner' + idx).css('display', 'block');
	...
	//Botão renascer
		...
		$('.efeito_fade').append('<span class="hover"></span>').each(function () {
	  		var $span = $('span.hover').css('opacity', 0);
	  		$('.efeito_fade').click(function () {
			$("span.hover").fadeIn(400);
				 $span.stop().fadeTo(100, 1);
			$("span.hover").fadeOut(300);
	  		});
