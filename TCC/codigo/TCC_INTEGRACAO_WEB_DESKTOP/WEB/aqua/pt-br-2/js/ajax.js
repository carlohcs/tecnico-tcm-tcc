$(function(){
	...
	//BUSCA DO USUÁRIO
	var pageBus = $('#pageBus').val();
	var clone = ""; var idEsc = "";
	switch(pageBus)
	{
		case "index";
				clone = $('#center_cp').clone();
				idEsc = 'center_cp';
			break;
		case "perfil";
				clone = $('#center').clone();
			...
			break;	
			case "center";
				clone = $('#box_center').clone();
				var tit = $('#title_game').text();
				idEsc = 'center_cp';
			break;
	}
	
	var tit = $('#title_game').text(); clone = $('#box_center').clone();
	$('#c_search').keydown(function(){
				...
				$("#box_center").html("<p>::ARTIGOS::</p>");
					...
					if($(data).find('artigos item url:first').text() != 0)
					{
						$(data).find('artigos item').each(function(i){
							...
							html += "<li class='gallery'><a href='artigos/" + lnkAtu + "'>" + titAtu + " - " + dtAtu + "</a></li>";
		...
		}else if($(this).val().length == 0){ $("#box_center").html(clone); $('#title_game').text(tit);}
		});	
	//----BUSCA DO USUÁRIO	
	...
	$('#comment a').click(function(e){
		...
		if($(this).attr('href') == 'comentarArtigo')
			...
			var tpComt = 'comentarioNoticia';
			 else var tpComt = 'comentarioDestaque';
		
		$('#box_center').append('<textarea id="txtareaComment" name="txtareaComment"></textarea><span id="cont"></span><input type="button" value="Enviar Comentário" id="enviComment" />');
		$('#txtareaComment').focus();
		 $('#enviComment').click(function(){
			$.ajax({
				url: "server/php.class/retornaDadosClasseAjax.php",
				dataType: "text",
				type: "POST",
	 ...
	 $('.curtir').click(function(){
		...
		$.ajax({
			...
			success: function(data){ 	
				if(data == 'true'){
					if(okCurt == 'true'){
						$('.curtir').text("Desfazer Curtir!");
						window.location.reload(true);
					}else{
						$('.curtir').text("Curtir Artigo!"); $('#statusCurtir').val('true');
						window.location.reload(true);
					} 
				}else $(this).text("Erro!");},
