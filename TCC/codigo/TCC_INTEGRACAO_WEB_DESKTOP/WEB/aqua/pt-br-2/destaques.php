	...
	<div id="box_center">
		...
		<?php
			...
			switch($itn)
			{
				...
				case 'ana':
					echo "<p>{$resp[0]}</p>";
					echo '<p><input type="button" value="comentarDestaque" id="comment"/></p>';
				break;
				case 'img':
			 ...
			 $objComt->exibeComentarioEcho($codDest,'destaque');	$permCmt = ($verificaSess == true) ? 'true' : 'false';
			
			
			
			
		?>
		<p><input type="hidden" value="<?php echo $codDest; ?>" id="codPost" />
			...
			<?php include_once('include/include_sec/sharer.php'); ?>
	</div>
