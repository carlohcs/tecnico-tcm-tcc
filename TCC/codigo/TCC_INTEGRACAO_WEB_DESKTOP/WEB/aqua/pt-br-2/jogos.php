					...
					<?php
						switch($getActJ)
						{
							...
							case 'descricao':
								...
								echo "<p><img src='{$li['PATH_IMG1_DESC_INT_CTG']}' width='420px' height='126px'/></p>";
								echo "<p>{$li['TXT_ITN_CTG']}</p>";
							break;
							case 'download':
								echo "<p class='t_down'><strong>Download</strong></p>
									<p><strong><a href='' target='_blank'>Clique aqui</a></strong> para fazer o download.</p>
							...
							case 'imagens':
								...
								echo "<ul>";
								for($i=0;$i<count($li);$i++){ if($li[$i] != "") echo "<li class='gallery'><a  href='".$li[$i]."' rel='clbx' title='".$url.$i."'><img class='gallery_img' src='".$li[$i]."' alt='Imagem' rel='cl' /></a></li>"; }
								echo "</ul>";
							...
							case 'videos':
								...
								if($li[0] != '') $video = $li[0]; else if($li[1] != "") $video = $li[1]; else $video = $li[3]; 
									...
									echo "</div>";
								}else echo "<p>Não existe vídeos para esse jogo.</p>";
