...
class manageNews extends sqlInstruction
{
	...
	function noticiasTexto($boo,$qtd,$qtdAtual,$booNormal)//boo == true -> noticia resumida com "..." else noticia normal 
	{
		...
		{
			if($booNormal == "normal" ) $objRespo = parent::selectSql("LINK_NOT,TIT_NOT,DT_NOT", "", 'DT_NOT DESC', '7',false);
	...
	function newsArtigo($boo) //$boo = artigo = resumido TO_DAYS ( NOW() ) - TO_DAYS( artigo.DT_POST_ART) AND 
	{
		...
		if(parent::SQLSERVER == true)
			$objRespo = parent::selectSql("artigo.COD_ART,artigo.LINK_ART,artigo.TIT_ART,artigo.DT_POST_ART, usuario.LOG_USU,categoria.TIT_CTG", "usuario.COD_USU = artigo.COD_USU AND FLAG_ART_AP = 1", 'DT_POST_ART DESC', '30',false);
		else
