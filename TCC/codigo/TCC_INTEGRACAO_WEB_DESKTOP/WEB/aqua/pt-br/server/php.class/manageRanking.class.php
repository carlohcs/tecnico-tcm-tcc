...
class manageRanking extends sqlInstruction
{
	...
	function atualizaRanking()
	{
		...
		if($exSql != false) //se encontrar registro
		{
			...
			if($dtAt > $intDtBd)
			{
				...
				$arrDados = parent::selectSql('COD_USU, PTS_USU','','PTS_USU DESC','',false);//seleciona os pontos e códigos dos usuários
				...
				if(parent::SQLSERVER == true)
				{
					while($ret = mssql_fetch_assoc($arrDados))
					{
						...
						$stringQuery.= "('{$ret['COD_USU']}','{$ret['PTS_USU']}' ,NOW())";
				...
				{
					while($ret = parent::fetch($arrDados))
					{
					
						$stringQuery.= "('null','{$ret['COD_USU']}','{$ret['PTS_USU']}' ,NOW())";
						if($i <  mysql_num_rows($arrDados)) $stringQuery.= ', ';
						$i++;
	...
	function exibeRanking()
	{
		...
		$i=0; $dtRnk = '';
		$html = ''; $qtdusu=0;
		$html.= "<h1 id='t_ranking'><img src='img/medal.png' alt='medalha' class='medal'/> Posição<span><img src='img/star.png' alt='pontos' class='medal'/> Pontuação</span></h1><table class='ranking'>";
		
	
			while($retorno = parent::fetch($exSql))
			{
				$i++;
				if($dtRnk != $retorno['DT_RNK']) $html.= "<p class='dataRnk'>Última semana (".parent::formataData($retorno['DT_RNK']).")</p>";
					$html.= "<td><span class='deg'>{$i}&deg; </span> <a href='perfil/{$retorno['LOG_USU']}'>{$retorno['LOG_USU']}</a></td>";
					$html.= "<td class='rank_point'>{$retorno['PTS_USU']}</td></tr>";
		...
		if($exSql != false)
		{
			...
			for($i=0,$j=0;$i<count($arrDados['datas']),$j<count($arrDados['logins']);$i++,$j++)
			{
				...
				$html.= "<p class='dataRnk'>Data: ".parent::formataData($arrDados['datas'][$i])."</p>";
				...
				for($k=0;$k<$qtdusu;$k++)
				{
					$html.= "<tr><td class='deg'>".($k + 1)."&deg; <a href='perfil/{$arrDados['logins'][$j][$k]}'>{$arrDados['logins'][$j][$k]}</a></td>";
					$html.= "<td class='rank_point'>{$arrDados['pontos'][$pontos]}</td></tr>";
					$pontos++;
				}
	...
	function exibePosicaoUsuarioRanking($codUsu)
	{
		...
		else
		{
			...
			$arrRetorno['posicoes'][0] = "---";
			$arrRetorno['posicoes'][1] = "---";
			$arrRetorno['posicoes'][2] = "---";
			$arrRetorno['datas'][0] = "---";
			$arrRetorno['datas'][1] = "---";
			$arrRetorno['datas'][2] = "---";
			$arrRetorno['pontos'][0] = "---";
