<?php if(basename($_SERVER["PHP_SELF"]) == "contato.php") header("Location: ./contato");
		...
		$objData->retornaDadosSessaoUsuario('contato');		
		$nome = $objData->exibeArrayDados('nome','usuario');$email = $objData->exibeArrayDados('email','usuario');
		//Realiza o CONTATO
		if(isset($_POST['sendContact'])){
			$retornoObj = $objData->contatoUsuario($_POST['nameContact'],$_POST['emailContact'],$_POST['selectContact'],$_POST['messageContact']);
			...
			$mensagem = ($retornoObj['dados'][3] !="") ? $retornoObj['dados'][3] : FALSE;$mensagemMen = ($retornoObj['mensagem'][3] !="") ? $retornoObj['mensagem'][3] : FALSE;}
				...
				<div id="center">
					...
					<div id="top2"></div>
					<div id="box_center">
						<form id="frmContact" method="post" action="./contato" class="frmData">
							<fieldset>
								...
								<dl>
									...
									<dd><textarea name="messageContact" id="mensagem"><?php echo $mensagem; ?></textarea><span id="cont"></span></dd>
