...
switch($getPage)
{
	...
	case 'artigos':
		if($verificaSess == false){
			echo "<script>$(document).ready(function(){ jAlert('<p>Você precisa estar logado para visualizar artigos!</p>Faça login ou cadastra-se.', ' ', function(){	$.alerts.dialogClass = null;})});</script>";
			include('include/body/center.php');
		}else{		
	...
	//TUTORIAIS
		...
		echo "<script>document.title = 'Tutoriais - JOI&D';</script>";
	break;
		case 'grand_chase_tutorial':
		include("grand_chase_tutorial.php");
		echo "<script>document.title = 'Grand Chase Error NSIS - JOI&D';</script>";
	break;
	//TODOS OS ARTIGOS
	case 'todos-artigos':
