...
$(document).ready(function(){
	...
	$('#bg_cbanner ul').cycle({
		...
		before: function(atual, proximo, opcoes, avancando){
			...
			//$('#btn_cbanner'+teste).css({'display':'block'})
			
			//Pega o elemento ativo (null na primeira)
			var teste = $('.activeSlide a').html()
			//if (teste == 'null') $('#btn_cbanner1').css({'display':'block'})
			if(teste == 4) {
				teste=0;
				$('#btn_cbanner4').css({'display':'none'});
			}
			...
			teste--;
			$('#btn_cbanner'+teste).css({'display':'none'})
	...
	$('#btn_cbanner li a').click(function(){
	...
	$("#ul_faq li.resu").next("li").hide(250);
	$(".tit").click(function(){
		$("#ul_faq li.resu").show(250);
		$("#ul_faq li").not(".tit").not(".resu").hide(250);
		$(this).next('.resu').toggle(250,
			function(){
				$(this).next('li').toggle(250);
			});
	});
	...
	//UPLOAD FOTO
	 $('#change_img_link').click(function(){ $.containerShow({file: 1, title: "UPLOAD FOTO"}); });
	
	var codEstSl = $('#codEstSl').val();
	$('#slcEst').find("option[value="+codEstSl+"]").attr('selected','selected');
	
	//SINALIZAÇÃO
	$('.sinalizar').click(function(){
	
		var optSin = $(this).val();
		$.containerShow({file: 2, title: "SINALIZAR USUÁRIO", data: optSin});
		
	});
	
	$('.btn_down_more').click(function(e){ e.preventDefault(); $('ul.menu_usuario').slideToggle('medium'); });
	
	//ESCREVER ARTIGO
	$('#l_left1 li').eq(3).click(function(e){
		e.preventDefault();
		if($('#usLog').val() != 'false'){
			$.containerShow({file: 0, title: "ESCREVER ARTIGO"});
		}else {
			jAlert('<p>Faça login ou registre-se para escrever artigos.</p>', ' ', function(){$.alerts.dialogClass = null;});		
		}
	});
	
	//EXLUIR USUÁRIO
	$('.menu_usuario li:eq(3) a').click(function(e){
		e.preventDefault();
		jAlert('<p>Você tem certeza que deseja excluir sua conta?</p>', ' ', function(){
			window.location.href = "http://127.0.0.1:88/TCC/aqua/pt-br/excluir-conta";
		}, 'visible');
	});
	//--EXLUIR USUÁRIO 
	 
	 
	/*var codCidSl = $('#codCidSl').val();
	$('#slcCid').append("<option value="+codCidSl+"></option>").attr('selected','selected');*/
	
						
	//Limitador de caracteres para comentário
	//$("textarea[name='txtareaComment']").limit('140', '#cont');	
	
	//Limitador de caracteres para descrição de usuário
	//$("textarea[name='messageRegister']").limit('140', '#cont');	
	
		/*$("#l_login").hover(function(){
        $(this).find("ul.menu_usuario").stop(true, true).slideDown(400);
    }, function(){
        $(this).find("ul.menu_usuario").stop(true, true).slideUp(400);
	});*/
