    ...
    public partial class PubForm : Form
    {
        ...
        {            
         ...
         public void infoinsert(Int32 xx, DataSet ds1) {
             ...
             DataRow dRow;
             if (ImgBox4.Size == new Size(167, 269)) { dRow = ds1.Tables[0].Rows[xx + 4]; }
             else { dRow = ds1.Tables[0].Rows[xx]; }
             InfoLabel.Text = dRow.ItemArray.GetValue(2).ToString();
         }

         private void info1_Click(object sender, EventArgs e)
         {
             DataSet ds1 = new DataSet();
             ds1 = class1.databaseconnection("select * from imagelist");
             infoinsert(0, ds1);
         }
